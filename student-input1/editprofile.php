...
<header>
    ...
    <div class="tab-pane fade show" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab" tabindex="0" style="width: 400px; height: 310px; font-size: .8rem;">
        <div class="shadow p-4 rounded-3">
            ...
            $getData = $conn->prepare("SELECT * FROM user_register WHERE id = ?");
            $getData->execute([$id]);
                        ...
                        <div class="position-relative mb-3">
                            <div class="mb-1 row">
                                <div class="col-5 py-1">
                                    <label for="first" class="form-label "><b>Full Name:</b></label>
                                </div>
                                <div class="col" id="input">
                                    <input type="hidden" class="form-control" name="id" value="<?= $selects['id'] ?>">
                                    <input type="text" style="font-size: .7rem;" class="form-control" id="first" name="fname" value="<?= $selects['full_name'] ?>">
                                </div>
                            </div>
                            <div class="mb-1 row">
                                <div class="col-5 py-1">
                                ...
                                <div class="col" id="input">
                                    <input type="text" style="font-size: .7rem;" class="form-control" id="last" name="email" value="<?= $selects['email'] ?>">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="my-3 form-check card-body text-center">
                        <button type="submit" class="btn btn-warning text-white" name="updateProfile">Update</button>
                    </div>
                </form>
                <?php } ?>
            <?php } ?>
